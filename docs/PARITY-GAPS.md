...
Capabilities present in the original Go server that are **missing or stubbed** in
the C# rewrite. Verified against the codebase on 2026-06-30. Go file paths below
are relative to the repository root at the `go-final` tag, which is where the Go
tree lives now that `legacy/` has been deleted.
...
- **Admin JSON API + API-key auth.** Go: `/api/admin/*` (`internal/api/admin.go`,
...
- **Abuse-report flow** (DMCA/takedown). Go: `internal/web/abuse.go`.

## Tier 3 — niche / Azure-specific

- **Entra ID Postgres auth** (`internal/db/entra_postgres.go`) — ported: append
...
- **App-level blob encryption at rest (`RSTASH_BLOB_KEY`).** Go wrapped blobs in
  an app-managed AES store (`internal/blob/encrypted.go`, `keyprovider.go`).
...
- **Webhooks** (`internal/webhooks/`). Existed to drive billing/integration for
