...
func SettingDefs() []SettingDef {
	return []SettingDef{
		...
		},
		{
			...
			Help:            "The Data Source Name for the metadata database, set via the " + EnvDB + " environment variable. Stores users, sessions, OAuth tokens, audit entries, and file metadata (nodes). Supported formats: sqlite:path (e.g. sqlite:rstash.sqlite), sqlite::memory:, postgres:host=localhost dbname=rstash user=rstash password=secret sslmode=disable, postgres://user:pass@host:5432/dbname?sslmode=require, mysql:user:pass@tcp(host:3306)/dbname?parseTime=true, mssql:sqlserver://user:pass@host:1433?database=dbname. For Azure Database for PostgreSQL, pass auth=entra in the query string to authenticate via Entra ID / managed identity instead of a password: postgres://principal@server.postgres.database.azure.com:5432/rstash?sslmode=require&auth=entra. In that mode rstash fetches access tokens via the DefaultAzureCredential chain (env service principal, managed identity on App Service / VM / Functions, Azure CLI) and refreshes them automatically as pool connections cycle. The Postgres user in the DSN must match the AAD principal the token is issued for; grant that principal access in Azure (Server → Security → Authentication → Add Entra admin / role). Changing this setting requires a server restart.",
		...
		},
		{
			...
			Help:            "The Data Source Name for the blob (file content) store, set via the " + EnvBlob + " environment variable. Supported backends: sqlite:path stores blobs in a SQLite database (simple, single-file), fs:/path/to/dir stores blobs as individual files on the filesystem (better for large deployments), postgres:/mysql:/mssql: store blobs in the corresponding database (useful when running on a non-SQLite metadata database), s3:bucket?region=us-east-1&endpoint=s3.amazonaws.com&prefix=optional/prefix stores blobs in an S3-compatible object store, and azureblob:container?account=NAME stores blobs natively in Azure Blob Storage. S3 credentials can be set via access_key/secret_key query parameters or AWS_ACCESS_KEY_ID/AWS_SECRET_ACCESS_KEY environment variables. S3 examples: s3:my-bucket?region=us-west-2 (AWS), s3:my-space?region=nyc3&endpoint=nyc3.digitaloceanspaces.com (DigitalOcean Spaces), s3:data?endpoint=minio.local:9000&tls=false&access_key=minioadmin&secret_key=minioadmin (MinIO). Azure Blob authentication: pass explicit credentials in the DSN via key= (shared key) or sas= (SAS token); otherwise rstash uses Azure's DefaultAzureCredential chain (env service principal, managed identity on App Service / VM / Functions, Azure CLI). On Azure App Service with a system-assigned identity granted the Storage Blob Data Contributor role, just azureblob:CONTAINER?account=NAME is enough — no secrets in config. Azure Blob examples: azureblob:rstash?account=rstprdsteus2001 (App Service with managed identity), azureblob:rstash?account=myacct&key=BASE64KEY== (shared key), azureblob:rstash?account=devstoreaccount1&key=Eby8vdM02xNOcqFlqUwJPLlmEtlCDXJ1OUzFT50uSRZ6IFsuFq2UVErCz4I6tq/K1SZFPTOtr/KBHBeksoGMGw==&endpoint=127.0.0.1:10000&tls=false (Azurite emulator). The S3 bucket or Azure container must exist before starting the server. Changing this setting requires a server restart.",
		...
		// ── Storage (runtime-editable, no env var) ──
		{
			...
			Help:            "The maximum total storage allowed across all users combined. Enforced on every PUT. Set to 0 to disable the global cap (recommended for self-hosters). Accepts human-readable sizes: B, KB, MB, GB, TB (binary units, 1 KB = 1024 bytes). Examples: 50GB, 500MB, 1TB. Changes take effect immediately.",
			Default:         "0",
		...
		},
		{
			...
			Help:            "The storage limit applied to each new user account at creation time. Existing users are not affected when this value changes — their limit is already set on their account and can be edited individually. Set to 0 to leave new users unlimited. Accepts human-readable sizes: B, KB, MB, GB, TB. Changes take effect for the next user created.",
			Default:         "0",
			InputType:       InputByteSize,
			RuntimeEditable: true,
		},
		...
		},
		{
			...
			Help:            "The maximum total monthly download (egress) transfer across all users combined. Enforced on every read. When exceeded, GetDocument returns 429 with a Retry-After header pointing to the next calendar-month boundary (UTC). Set to 0 to disable the global cap. Accepts human-readable sizes: B, KB, MB, GB, TB. Changes take effect immediately.",
			Default:         "0",
			InputType:       InputByteSize,
			RuntimeEditable: true,
		},
		{
			...
			Help:            "The monthly download (egress) limit applied to each new user account at creation time. Existing users are not affected when this value changes — their limit is already set on their account and can be edited individually. Set to 0 to leave new users unlimited. Resets at the calendar-month boundary (UTC). Accepts human-readable sizes: B, KB, MB, GB, TB. Changes take effect for the next user created.",
			Default:         "0",
			InputType:       InputByteSize,
			RuntimeEditable: true,
		},
