...
func SettingDefs() []SettingDef {
	return []SettingDef{
		...
		},
		{
			...
			Group:           "Storage",
			Label:           "Egress mode",
			Description:     "How monthly egress (download) transfer is enforced.",
			Help:            "Controls monthly egress limits per user. \"off\" disables both tracking and enforcement — no counter writes on the read path. \"user\" enforces per-user limits (configured via egress_quota_user, with optional per-user overrides via the admin API). When a user's remaining quota for the current calendar month is zero, GetDocument returns 429 with a Retry-After header pointing to the next-month boundary. Uploads always succeed — ingress is not tracked here. Changes take effect immediately.",
			Default:         "user",
		...
		{
			Key:             "egress_quota_user",
			Group:           "Storage",
			Label:           "Per-user egress",
			Description:     "Default monthly egress (download) limit per user (e.g. 500GB).",
			Help:            "The default monthly egress (download) limit per user when egress_mode=user. Individual users can be given higher or lower limits via the admin API. Accepts human-readable sizes: B, KB, MB, GB, TB. Must be greater than 0. Resets at the calendar-month boundary (UTC). Changes take effect immediately.",
			Default:         "500GB",
			InputType:       InputByteSize,
			RuntimeEditable: true,
		},
